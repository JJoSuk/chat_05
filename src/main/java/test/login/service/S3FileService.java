...
import test.login.dto.FileUploadDto;
import test.login.entity.ChatFile;
import test.login.entity.ChatRoom;
import test.login.repository.ChatFileRepository;
    ...
    public void deleteFileDir(String roomId) {
        ...
        if (chatRoom != null) {
            ...
            for (ChatFile file : files) {
                try {
                    log.info("Attempting to delete file with S3 key: " + file.getS3Key());
                    amazonS3.deleteObject(bucket, file.getS3Key());
                    chatFileRepository.delete(file);
                    log.info("Successfully deleted file with S3 key: " + file.getS3Key());
                } catch (Exception e) {
                    log.error("Error deleting file with S3 key: " + file.getS3Key(), e);
                }
            }
        } else {
            log.warn("ChatRoom with ID " + roomId + " not found.");
        }
    }
